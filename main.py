...
if __name__ == '__main__':
    ...
    # text = text[:1000]
    # 划分训练集测试集
    x_train, x_test = train_test_split(text[:], test_size=0.3, stratify=text['index'])

    # import model
    tokenizer = BertTokenizer.from_pretrained('D:\\学校\\学年论文\\1.16\\chinese_rbtl3_pytorch')
    model = BertForSequenceClassification.from_pretrained('D:\\学校\\学年论文\\1.16\\chinese_rbtl3_pytorch', num_labels=16).to(device)
